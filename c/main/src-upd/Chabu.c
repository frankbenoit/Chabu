...
#if defined(FEATURE_MODPROT2)


#define FEATURE_DEFENSIVE_PROGRAMMING

#include <stdint.h>
#include "Common.h"
#include "Chabu.h"
#include "UdpState.h"
#include "util/QueueVar.h"
...
void Chabu_Init(
	...
	data->connectionInfoLocal.v1.applicationNameLength = strnlen(applicationName, Chabu_APPLICATION_NAME_SIZE_MAX);
...
static bool calcNextRecvChannel( struct Chabu_Data* data ) __attribute__((unused));
static bool calcNextRecvChannel( struct Chabu_Data* data ) {
	int priority = INT32_MIN;
	struct Chabu_Channel_Data * foundChannel = NULL;
	int chIdx = ( data->recvChannelIdx +1 );
	if( chIdx >= data->channelCount ){
		chIdx = 0;
	}
	int i;
	for( i = 0; i < data->channelCount; i++ ){
		struct Chabu_Channel_Data * channel = data->channels[chIdx];
		Chabu_Assert( channel != NULL );
		if( channel->recvRequest && priority <= channel->priority ){
			foundChannel = channel;
			priority = channel->priority;
		}
		chIdx++;
	...
	if( foundChannel ){
		foundChannel->recvRequest = false;
		data->recvChannelIdx = foundChannel->channelId;
		return true;
	}
...
#define DEBUG_CHABU_PROTOCOL
#ifdef DEBUG_CHABU_PROTOCOL
int last_packet_pay_load_size = 0;
int32 last_seq = 0;
int last_return = __LINE__;
#endif
...
static int Chabu_Channel_handleRecv( struct Chabu_Data* chabu, struct Chabu_Channel_Data* channel, void* recvData, int length ){
...
#ifdef DEBUG_CHABU_PROTOCOL
				dbg_printf( "wrong packet flag %d, last_packet_pay_load_size: 0x%x, last packet seq: 0x%x, last_return = %d",
						pkf, last_packet_pay_load_size, last_seq, last_return );
#endif
				dbg_memory("recvData", recvData, length );
				Chabu_Assert(false);
				return 0;
			}
		}
	...
	if( chabu->recvLastIndex < 0 ){
		dbg_printf("channel->channelId %d, channel->recvLastIndex %d, channel->recvLastLength %d", channel->channelId, chabu->recvLastIndex, chabu->recvLastLength );
	}
...

	{
		...
		if( remainingTrg > remainingSrc ){
			...
			// because only armed payload is transmitted
			if(remainingTrg == 0){
				dbg_printf("remainingSrc: %d = length: %d - res: %d, remainingTrg: %d = chabu->recvLastLength: %d - chabu->recvLastIndex: %d",
						remainingSrc, length, res, remainingTrg, chabu->recvLastLength, chabu->recvLastIndex);
				dbg_memory("recvData", recvData, length );
			}
...
void Chabu_PutRecvData ( struct Chabu_Data* data, void* recvData, int length ){
	...
	while( idx < length ){
		...
		// AA = ARM position
		{
			...
			if( channelId >= data->channelCount ){
				...
				dbg_printf( "idx %d, unexpected channelId 0x%x, flag 0x%x, seq/arm 0x%x, last_packet_pay_load_size: 0x%x",
						idx,
						channelId,
	...
	if(idx != length){
		dbg_printf( "idx %d != length %d, last_packet_pay_load_size: 0x%x",
				idx, length, last_packet_pay_load_size );
		dbg_memory("recvData", recvData, length );
...
//FIXME: remove, wrong test
//			if( pls > 0xFFFF ){
//				dbg_printf( "pls %d < 0xFFFF, xmitMaxPayloadSize: %d, wr_idx:%d, rd_idx: %d, buf_size: %d",
//						pls,
//						channel->chabu->xmitMaxPayloadSize,
//						channel->xmitQueue->wr_idx,
//						channel->xmitQueue->rd_idx,
//						channel->xmitQueue->buf_size);
//			}
//
//			Assert( pls >= 0 && pls <= 0xFFFF );
	...
	if( chabu->xmitLastLength != 0 ){
		...
		if(( chabu->xmitLastIndex >= Chabu_HEADER_SIZE_MAX )
			...
			if( !( ( copySz >= 0 ) && (copySz <= size ) ) ){
				dbg_printf(
					"copySz: %d, remainingTrg: %d, remainingSrc: %d, QueueVar_Available( %s ): %d",
					copySz, remainingTrg, remainingSrc, channel->xmitQueue->name, size );
			}

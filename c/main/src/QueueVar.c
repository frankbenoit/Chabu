...
void QueueVar_Clear( struct QueueVar* queue ){
	...
	queue->wr_idx = 0;
	queue->rd_idx = 0;
	TQueueVar_NotifyConsumed* callbackConsumed = queue->callbackConsumed;
	if( callbackConsumed ){
		callbackConsumed( queue->callbackConsumedData, queue, sz );
	}
}
...
void QueueVar_Write( struct QueueVar* queue, const void* buf, int len ){
	...
	int rd = queue->rd_idx; // get rd_idx before writing wr_idx, to avoid race condition for check
	queue->wr_idx = wr;
	if( len ){
		AssertPrintf( rd != wr, "after adding data, they cannot be equal: %d %d", rd, wr );
	}

	TQueueVar_NotifySupplied* callbackSupplied = queue->callbackSupplied;
	if( callbackSupplied ){
		callbackSupplied( queue->callbackSuppliedData, queue, len );
	}
...
void QueueVar_Peek( struct QueueVar* queue, void* buf, int len ){
	...
	if( ! ( ( rd+remaining <= wr ) || (wr < rd) ) ){
		Chabu_dbg_printf( "Queue %s, wr:%d rd:%d remaining:%d", queue->name, wr, rd, remaining );
	}
...
void QueueVar_ReadCommit( struct QueueVar* queue, int len ){
	...
	TQueueVar_NotifyConsumed* callbackConsumed = queue->callbackConsumed;
	if( callbackConsumed ){
		callbackConsumed( queue->callbackConsumedData, queue, len );
	}
